...
torch.save(best_model.state_dict(),'Balanced_epoch[%d]_learning_rate[%.4f]_simpler_2.pth'%(best_epoch,best_lr))

File = h5py.File('simpler_train.h5','w')
File['data'] = training_error
File.close()
File = h5py.File('simpler_valid.h5','w')
File['data'] = validation_error
File.close()
File = h5py.File('simpler_test.h5','w')
